...
class BST:
    ...
    def __contains__(self, key):
        ...
        # your code here
        return self._contains_aux(self._root, key)
    ...
    def _height_aux(self, node):
        ...
        height = 0
        if node is not None:
            height = 1 + max(self._height_aux(node._left), self._height_aux(node._right))
        return height
    ...
    def _eq_aux(self, node1, node2):
        ...
        ---------------
        Parameters:
            node1 - a bst node (BST)
            node2 - a bst node (BST)
        Returns:
            equals - True if source contains the same values
                as target in the same location, otherwise False. (boolean)
        -------------------------------------------------------
    ...
    def _parent_aux(self, node, key):
        ...
        if node is not None:
            if node._left is not None and node._left._value == key:
                ...
                value = self._parent_aux(node._left, key)
            else:
                value = self._parent_aux(node._right, key)
        return value
    ...
    def node_counts(self):
        ...
        # your code here
        zero = self._zero_child_count_aux(self._root)
        one = self._one_child_count_aux(self._root)
        two = self._two_child_count_aux(self._root)
        return zero, one, two
    
    def _zero_child_count_aux(self, node):
        """
        ---------------------------------------------------------
        Returns the number of the three types of nodes in a BST.
        Use: count = bst.zero_child_count()
        -------------------------------------------------------
        Returns:
            count - number of nodes with zero children in bst (int)
        ----------------------------------------------------------
        """
        ...
        if node is None:
            ...
            count = self._zero_child_count_aux(node._left) + \
                self._zero_child_count_aux(node._right)
        return count
    ...
    def _is_balanced_aux(self, node):
        """
        ---------------------------------------------------------
        Determines whether a bst is balanced, i.e. the difference in
        height between all the bst's node's left and right subtrees is <= 1.
        Use: balanced = self._is_balanced_aux(node)
        ---------------------------------------------------------
        Parameters:
            node - a bst node (_BST_Node)
        Returns:
        ...
        if node is None or node._height == 1:
            ...
            balanced = self._is_balanced_aux(node._right) and self._is_balanced_aux(node._left)
        return balanced
    ...
    def levelorder(self):
        ...
        values = []
        if self._root is not None:
            values = self._levelorder_aux(self._root)
        return values

    def _levelorder_aux(self, node):
        """
        -------------------------------------------------------
        Copies the contents of the tree in levelorder order to a list.
        Use: values = bst._levelorder_aux(node)
        -------------------------------------------------------
        Parameters:
            node - a bst node (_BST_Node)
        Returns:
    ...
    def append(self, value):
        """
        -------------------------------------------------------
        Appends contents of source to the end of the current target
        list. At finish source contains nothing.
        Use: target.append(source)
        -------------------------------------------------------
        Parameters:
            source - an linked list (LinkedList)
        Returns:
            None
        -------------------------------------------------------
        """
        ...
        if self._root is None:
            self._root = _BST_Node(value)
            self._count += 1
        else:
            self._append_aux(self._root, value)
        return

    def _append_aux(self, node, value):
        """
        -------------------------------------------------------
        Appends contents of source to the end of the current target
        list. At finish source contains nothing.
        Use: target._append_aux(source._root)
        -------------------------------------------------------
        Parameters:
            source - an linked list (LinkedList)
        Returns:
        ...
        if value < node._value:
            if node._left is None:
                node._left = _BST_Node(value)
                self._count += 1
            else:
                self._append_aux(node._left, value)
        elif value > node._value:
            if node._right is None:
                node._right = _BST_Node(value)
                self._count += 1
            else:
                self._append_aux(node._right, value)
        return
