...
def fill_letter_bst(bst, values):
    ...
    for i in range(len(values)):
        letter = values[i][0]
        code = values[i][1]
        letter_code = ByLetter(letter, code)
        bst.insert(letter_code)
...
def fill_code_bst(bst, values):
    ...
    for i in range(len(values)):
        letter = values[i][0]
        code = values[i][1]
        letter_code = ByCode(letter, code)
        bst.insert(letter_code)
...
def encode_morse(bst, text):
    ...
    for char in text.strip():
        if char.isalpha():
            letter = ByLetter(char.upper(), "")
            node = bst.retrieve(letter)
            if node is not None:
                result += node.code + " "
        elif char.isdigit():
            letter = ByLetter("_", char)
            node = bst.retrieve(letter)
            if node is not None:
                result += node.code + " "
        elif char == " ":
